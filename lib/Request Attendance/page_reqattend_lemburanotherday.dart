...
class _RequestLemburAnotherDay extends State<RequestLemburAnotherDay> {
  ...
  getAttendanceCheck() async {
    final response = await http.get(Uri.parse(
        applink + "mobile/api_mobile.php?act=getAttendanceChecklemburanotherday&karyawanNo=" +
            widget.getKaryawanNo+"&getDate="+widget.getDate)).timeout(
        Duration(seconds: 10), onTimeout: () {
      AppHelper().showFlushBarsuccess(context, "Koneksi terputus..");
      http.Client().close();
  ...
  _addreq_attend() async {
    ...
    final response = await http.post(
        Uri.parse(applink + "mobile/api_mobile.php?act=add_reqattend_lemburanotherday"),
        body: {
          ...
          "reqattend_starttime" : _TimeStart.text,
          "reqattend_endtime" : _TimeEnd.text,
            ...
            Container(
              ...
              Center(
                child: Column(
                    ...
                    children: [
                      Text("Oh Sorry", style: GoogleFonts.nunito(fontSize: 45,fontWeight: FontWeight.bold)),
                      Text("We Find Attendance in your date request", style: GoogleFonts.nunito(fontSize: 15)),
                      Text("You cant use this date", style: GoogleFonts.nunito(fontSize: 15))
                    ]),
              ) : getAttenceMessage == "4" ?
              Center(
                child: Column(
